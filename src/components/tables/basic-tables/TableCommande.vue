...
console.log(commandeStore);

onMounted(async () => {
  try {
    await commandeStore.fetchCommandes();
    data.value = commandeStore.commandes;
  } catch (error) {
    console.error('Erreur lors de la récupération des commandes :', error);
  }
});
const filteredData = computed(() => {
  return commandeStore.commandes.filter((datas) => {
    const clientMatch = !filters.value.client || 
      (datas.client?.name?.toLowerCase().includes(filters.value.client.toLowerCase()) ?? false);

    const numeroMatch = !filters.value.numeroFacture || 
      (datas.numeroFacture?.toString().toLowerCase().includes(filters.value.numeroFacture.toLowerCase()) ?? false);

    let dateMatch = true;
    if (filters.value.dateCommande) {
      const filterDate = new Date(filters.value.dateCommande).toDateString();
      const dataDate = new Date(datas.dateCommande).toDateString();
      dateMatch = filterDate === dataDate;
    }

    const etatMatch = !filters.value.etat || datas.etat === filters.value.etat;

    return clientMatch && numeroMatch && dateMatch && etatMatch;
  });
});
