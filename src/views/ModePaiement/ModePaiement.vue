        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 md:gap-4">
            ...
            <div
                ...
                <div class="flex items-end justify-between">
                    <div class="w-full">
                        <span class="text-lg font-bold text-gray-500 dark:text-gray-400">Nouveau mode</span>

                        <Input class="mt-2 h-11" v-model="modeName" placeholder="Nom du mode"></Input>
...
const currentPageTitle = ref("Mode de paiement");
...
const createmode = async () => {
    try {
        const response = await api.post("api/modePaiement/create", {
            name: modeName.value,
        });
        alertTitle.value = "Succès";
        alertVariant.value = "success";
        openAlert.value = true;
        message.value = response?.data?.message || "Une erreur est survenue lors de la création du mode.";

    } catch (error) {
        message.value = error.response?.data?.message || "Une erreur est survenue lors de la création du mode.";
