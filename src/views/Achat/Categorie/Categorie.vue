        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 md:gap-4">
            ...
            <div
                ...
                <div class="flex items-end justify-between">
                    <div class="w-full">
                        <span class="text-lg font-bold text-gray-500 dark:text-gray-400">Nouvelle categorie</span>

                        <Input class="mt-2 h-11" v-model="categorieName" placeholder="Nom du categorie"></Input>
                        <Button :onClick="createcategorie"
...
const currentPageTitle = ref("Categorie");
...
const createcategorie = async () => {
    try {
        ...
        const response = await api.post("api/categorie/create", {
            name: categorieName.value,
        });
        alertTitle.value = "Succès";
        alertVariant.value = "success";
        openAlert.value = true;
        message.value = "Produit cree avec success";

    } catch (error) {
        message.value = error.response?.data?.message || "Une erreur est survenue lors de la création du categorie.";
